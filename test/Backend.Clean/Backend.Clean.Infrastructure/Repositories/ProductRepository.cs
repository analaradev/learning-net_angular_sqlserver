...
namespace Backend.Clean.Infrastructure.Repositories;
...
public class ProductRepository : IProductRepository
{
    ...
    public async Task<List<(string Color, int ProductCount, decimal AveragePrice)>> GetProductsGroupedByColorAsync()
    {
        return await _context.Products
            .AsNoTracking()
            ...
            .ToListAsync()
            .ContinueWith(t => t.Result.Select(g =>
                (g.Color, g.ProductCount, g.AveragePrice)).ToList());
    }
    ...
    public async Task<(int trackedNormal, int trackedNoTracking)> GetTrackingComparisonDataAsync()
    {
        ...
        return (trackedEntitiesAfterNormalQuery, trackedEntitiesAfterNoTrackingQuery);
    }
